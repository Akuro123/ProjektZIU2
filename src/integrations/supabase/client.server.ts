...
function createSupabaseAdminClient() {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    const missing = [
      ...
      ...(!SUPABASE_KEY
        ? [
            "SUPABASE_SERVICE_ROLE_KEY, SUPABASE_PUBLISHABLE_KEY, or NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY",
          ]
        : []),
    ];
  ...
  if (!SUPABASE_SERVICE_ROLE_KEY) {
    console.warn(
      "[Supabase] SUPABASE_SERVICE_ROLE_KEY is missing; using publishable key with RLS.",
    );
  }
...
// SECURITY: Never expose a service role key to client code.
